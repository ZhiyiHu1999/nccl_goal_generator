...
rm npkit_data_summary_Simple.json

rm -rf "./plots"
mkdir -p "./plots"

export WORK_ROOT=/users/zhu/nccl_goal_generator/npkit_benchmark/clariden/Simple
...
srun --mpi=pmi2 --environment=megatron bash -c '
    ...
    if [ "$SLURM_NODEID" -eq 0 ]; then
        pip install seaborn
        python3 $WORK_ROOT/plot.py -i $WORK_ROOT/npkit_data_summary_Simple.json
